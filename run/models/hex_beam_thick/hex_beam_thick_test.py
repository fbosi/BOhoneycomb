...
def post_process(job_name):
    ...
    # field outputs for the last step
    step = odb.steps[odb.steps.keys()[-1]] # last step
    
    
    #all_frames = step.frames
    ...
    #all_RF = [frame.fieldOutputs['RF'] for frame in all_frames]
    #all_U = [frame.fieldOutputs['U'] for frame in all_frames]
    ...
    # All forces/displacements
    #all_forces = [RFALL.getSubset(region=RP).values[0].data[1] for RFALL in all_RF]
    #all_u2 = [UALL.getSubset(region=RP).values[0].data[1] for UALL in all_U]
    
    rel_density = (2.0/np.sqrt(3))*(t/l)*(1.0-(t/l)/(2.0*np.sqrt(3)))    
    scaling_stiffness = (1.5*(rel_density)**3.0)*young_modulus    
    scaling_strength = (0.5*(rel_density)**2)*yield_strength
            
    e11 = u1/(sqrt(3)*h)
    e22 = u2/(2*h)
    sim_results['v'] = -e11/e22
    ...
    #sim_results['strength_ratio'] = s22/scaling_strength
    sim_results['rel_density_target'] = rel_density
    odb.close()
...
create_sim("test_model","job",{'eta': 0.54, 'xi': 0.36},True)
results  = post_process("job")
print(results)
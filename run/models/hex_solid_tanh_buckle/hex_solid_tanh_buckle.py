...
t_uni = -np.sqrt(3)*(np.sqrt(1-relative_density)-1) # uniform thickness for the given relative density (uniform)


# Note here, that the uniform thickness is translated into t_H inside the CATIA file!! This ensure the compatibility, as t_H is dependent on the parametrisation
...
def create_sim(model_name,job_name,parametrization,save_cae=False):
    ...
    t_min = parameters.Item('t_min').Value/1000.0
